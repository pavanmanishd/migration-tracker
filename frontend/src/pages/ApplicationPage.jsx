...
const ApplicationPage = () => {
  ...
  const loadApplicationData = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getApplicationDetails(eaiCode);
      ...
      {loading ? (
        ...
        <>
          <Paper sx={{ p: 3, mb: 4 }}>
            ...
            <Box mt={2}>
              <Typography>
                {migrationItems.filter(item => item.status === 'COMPLETED').length} of {migrationItems.length} items completed
              </Typography>
            </Box>
          </Paper>

          <MigrationTable items={migrationItems} onItemUpdated={handleItemUpdated} />
        </>
      )}
    </Container>
  );
